...
public class StickerView extends FrameLayout {
  ...
  public void configDefaultIcons() {
    BitmapStickerIcon deleteIcon = new BitmapStickerIcon(
            //image 변경할것 : ic_delete
        ContextCompat.getDrawable(getContext(), R.drawable.ic_diary_delete),
  ...
  //show sticker
  @NonNull public StickerView showSticker(@NonNull Sticker sticker, @NonNull GetSticker stickerInfo) {
    sticker.setId(stickerInfo.getId());
    sticker.setIsNew(false);

    if (ViewCompat.isLaidOut(this)) {
      showStickerImmediately(sticker, stickerInfo);
    } else {
      post(new Runnable() {
  ...
  protected void showStickerImmediately(@NonNull Sticker sticker, GetSticker stickerInfo) {

    setStickerPosition(sticker, stickerInfo);
    stickers.add(sticker);
    if (onStickerOperationListener != null) {
      onStickerOperationListener.onStickerAdded(sticker);
    }

    Log.d("stickers", stickers.toString());

    invalidate();
  }

  protected void setStickerPosition(@NonNull Sticker sticker, GetSticker stickerInfo) {
    sticker.setFlippedHorizontally(stickerInfo.getFlipped());
    transformSticker(sticker, stickerInfo);

  }

  protected void transformSticker(@Nullable Sticker sticker, GetSticker stickerInfo) {
    if (sticker == null) {
      Log.e(TAG, "transformSticker: the bitmapSticker is null or the bitmapSticker bitmap is null");
      return;
    }
    ...
    float width = getWidth();
    float height = getHeight();
    float stickerWidth = (float) stickerInfo.getWidth();
    float stickerHeight = (float) stickerInfo.getHeight();
    //step 1
    float offsetX = (width - stickerWidth) / 2;
    float offsetY = (height - stickerHeight) / 2;
  ...
  @NonNull public StickerView addSticker(@NonNull Sticker sticker, int stickerId) {
    sticker.setStickerId(stickerId);
    sticker.setIsNew(true);
    return addSticker2(sticker, Sticker.Position.CENTER);
  }

  public StickerView addSticker2(@NonNull final Sticker sticker,
  ...
  protected void setStickerPosition(@NonNull Sticker sticker, @Sticker.Position int position) {
    ...
    Log.d("addSticker", sticker.getMatrix().toString());
  }
